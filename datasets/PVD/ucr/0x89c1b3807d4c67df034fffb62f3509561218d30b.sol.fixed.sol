...
function deliver (uint64    requestId,bytes32    paramsHash,uint64    error,bytes32    respData) public __lock_modifier0  {
if (msg.sender!=SGX_ADDRESS||requestId<=0||requests[requestId].requester==0||requests[requestId].fee==DELIVERED_FEE_FLAG)
{
return ;
}
...
{
externalCallFlag=true;
bool     __sent_result106 = requests[requestId].requester.call.gas(2300).value(fee)();
...
externalCallFlag=true;
bool     __sent_result103 = requests[requestId].callbackAddr.call.gas(callbackGas)(requests[requestId].callbackFID, requestId, error, respData);
