...
contract PandaBase is PandaAccessControl {
    ...
    function setTotalWizzPandaOf(uint256 _tp, uint256 _total)
        external
        onlyCLevel
    {
...
contract PandaBreeding is PandaOwnership {
    ...
    function _exchangeMatronSireId(uint256 _matronId, uint256 _sireId)
        internal
...
contract SaleClockAuction is sGuardPlus, ClockAuction {
    ...
    function surprisePanda() external payable {
        bytes32 bHash = keccak256(
            ...
            block.blockhash(sub_uint256(block.number, 1))
        );
        uint256 PandaIndex;
        if (bHash[25] > 0xC8) {
            require(uint256(RarePanda.length) >= RarePandaIndex);
            PandaIndex = RarePandaIndex;
            RarePandaIndex = add_uint256(RarePandaIndex, 1);
        } else {
            require(uint256(CommonPanda.length) >= CommonPandaIndex);
            PandaIndex = CommonPandaIndex;
            CommonPandaIndex = add_uint256(CommonPandaIndex, 1);
        }
    ...
    {
        common = sub_uint256(
            add_uint256(CommonPanda.length, 1),
            CommonPandaIndex
        );
        surprise = sub_uint256(
            add_uint256(RarePanda.length, 1),
            RarePandaIndex
        );
    }
...
contract SaleClockAuctionERC20 is ClockAuction {
    ...
    function erc20ContractSwitch(address _erc20address, uint256 _onoff)
        external
    {
        require(msg.sender == address(nonFungibleContract));
    ...
    function withdrawERC20Balance(address _erc20Address, address _to)
        external
        returns (bool res)
    {
        require(balances[_erc20Address] > 0);
        require(msg.sender == address(nonFungibleContract));
        bool __sent_result102 = ERC20(_erc20Address).transfer(
            _to,
            balances[_erc20Address]
        );
...
contract PandaAuction is PandaBreeding {
    ...
    function switchSaleAuctionERC20For(address _erc20address, uint256 _onoff)
        external
        onlyCOO
    {
        ...
        onlyCLevel
    {
        ...
        bool __sent_result103 = saleAuctionERC20.withdrawERC20Balance(
            _erc20Address,
            _to
        );
...
contract PandaCore is PandaMinting {
    ...
    function withdrawBalance() external onlyCFO {
        ...
        if (balance > subtractFees) {
            bool __sent_result105 = cfoAddress.send(balance - subtractFees);
