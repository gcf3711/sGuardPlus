...
contract CCindexToken is sGuardPlus,owned,token {
...
constructor ()  token(initialSupply, tokenName, decimalUnits, tokenSymbol)  {
}
...
for(uint     i = startIndex;i<lastIndex; i=add_uint(i, 1)){
address     holder = addresses[add_uint(i, 1)];
uint     reward = div_uint256(mul_uint256(balanceOf[holder], 3), 100);
balanceOf[holder]=add_uint(balanceOf[holder], reward);
distributed=add_uint(distributed, reward);
Transfer(0, holder, reward);
}

totalSupply=add_uint256(totalSupply, distributed);
return distributed;
}
