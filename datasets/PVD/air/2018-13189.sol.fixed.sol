...
contract UNLB is sGuardPlus,PausableToken {
...
balances[_x]=add_uint(balances[_x], _v);
totalSupply=add_uint256(totalSupply, _v);
Transfer(0x0, _x, _v);
}
...
owner=admin;
unlb=new UNLB ();
bool     __sent_result100 = unlb.pause();
require(__sent_result100);
}

function pricePerWei () public constant returns (uint    ){
if (now<1511799420)
return 800.0*1 ether;
 else 
if (now<1511885820)
return 750.0*1 ether;
 else 
if (now<1513181820)
return 675.0*1 ether;
 else 
if (now<1515514620)
return 575.0*1 ether;
 else 
if (now<1516205820)
return 537.5*1 ether;
 else 
return 500.0*1 ether;
...
bool     __sent_result101 = unlb.unpause();
