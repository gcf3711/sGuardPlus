...
contract Krown is sGuardPlus,owned,token {
...
constructor (uint256    initialSupply,string    tokenName,uint8    decimalUnits,string    tokenSymbol,address    centralMinter)  token(initialSupply, tokenName, decimalUnits, tokenSymbol)  {
if (centralMinter!=0)
centralAuthority=centralMinter;
...
if (add_uint256(balanceOf[_to], _value)<balanceOf[_to])
revert();
...
if (balanceOf[msg.sender]<notificationFee)
revert();

if (bytes (_reference).length>66)
revert();

if (bytes (_notes).length>64)
revert();

balanceOf[msg.sender]=sub_uint256(balanceOf[msg.sender], notificationFee);
balanceOf[centralAuthority]=add_uint256(balanceOf[centralAuthority], notificationFee);
