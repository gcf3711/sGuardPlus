...
bool     __sent_result100 = targetToken.transfer(_exchanger, _tokens);
...
function updateTargetToken (address    _target,uint256    _rate) public onlyOwner  {
rate=_rate;
...
constructor (string    _projectName,string    _tokenName,string    _tokenSymbol,uint256    _cap) public  {
require(_cap>0);
...
function newCrowdSale (string    _projectName,string    _tokenName,string    _tokenSymbol,uint256    _cap) public onlyOwner  {
require(_cap>0);
projectName=_projectName;
tokenName=_tokenName;
tokenSymbol=_tokenSymbol;
...
bool     __sent_result101 = token.mint(beneficiary, ethAmount);
...
function increaseCap (uint256    _cap_inc) public onlyOwner  {
require(_cap_inc>0);
cap=cap.add(_cap_inc.mul(decimal_num));
IncreaseCap(cap);
}

function decreaseCap (uint256    _cap_dec) public onlyOwner  {
require(_cap_dec>0);
...
function transferTargetToken (address    _owner,uint256    _value) public onlyOwner  returns (bool    ){
if (targetToken!=address (0))
{
