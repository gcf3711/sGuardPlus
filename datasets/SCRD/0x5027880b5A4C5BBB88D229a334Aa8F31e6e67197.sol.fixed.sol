...
contract HDLContract is sGuardPlus,Ownable {
...
event GoalReached ();
constructor (address    aVaultAddress,uint    aStart,uint    aPeriod,uint    aGoal,uint    aRate)   {
...
bool     __sent_result100 = token.mint(this, tokens);
require(__sent_result100);
bool     __sent_result101 = token.transfer(msg.sender, tokens);
...
bool     __sent_result102 = token.stopMinting();
...
return (now>_start)&&(now<(add_uint(_start, mul_uint(_period, 1 days))));
}
