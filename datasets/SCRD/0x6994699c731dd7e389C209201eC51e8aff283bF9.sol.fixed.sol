...
return uint8 ((add_uint(div_uint(timestamp, 86400), 4))%7);
}

function getHour (uint    timestamp)   returns (uint8    ){
return uint8 ((div_uint(div_uint(timestamp, 60), 60))%24);
}

function getMinute (uint    timestamp)   returns (uint8    ){
return uint8 ((div_uint(timestamp, 60))%60);
}
...
constructor (address    tokenAddress,uint256    noOfTokens,uint256    prPerToken)   {
dates[0]=1505520000;
dates[1]=1506470400;
dates[2]=1507161600;
dates[3]=1507852800;
...
if (tokensToGive*pricePerToken>weiSent)
tokensToGive -- ;
...
if (weiLeftOver>0)
{
bool     __sent_result100 = msg.sender.send(weiLeftOver);
