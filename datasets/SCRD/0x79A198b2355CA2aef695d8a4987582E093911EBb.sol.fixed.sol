...
event Birth (address    owner,uint256    botId,uint256    matronId,uint256    sireId,uint256    genes,uint256    birthTime);
event Transfer (address    from,address    to,uint256    tokenId);
struct Bot {
uint256     genes;
uint64     birthTime;
...
uint32 [14] public   cooldowns = [uint32 (1 minutes), uint32 (2 minutes), uint32 (5 minutes), uint32 (10 minutes), uint32 (30 minutes), uint32 (1 hours), uint32 (2 hours), uint32 (4 hours), uint32 (8 hours), uint32 (16 hours), uint32 (1 days), uint32 (2 days), uint32 (4 days), uint32 (7 days)];
...
function _createBot (uint256    _matronId,uint256    _sireId,uint256    _generation,uint256    _genes,address    _owner) internal  returns (uint    ){
...
Bot    memory _bot = Bot({genes: _genes, birthTime: uint64 (now), cooldownEndBlock: 0, matronId: uint32 (_matronId), sireId: uint32 (_sireId), siringWithId: 0, cooldownIndex: cooldownIndex, generation: uint16 (_generation)});
uint256     newBotId = bots.push(_bot)-1;
require(newBotId==uint256 (uint32 (newBotId)));
Birth(_owner, newBotId, uint256 (_bot.matronId), uint256 (_bot.sireId), _bot.genes, uint256 (_bot.birthTime));
...
function extCreateBot (uint256    _matronId,uint256    _sireId,uint256    _generation,uint256    _genes,address    _owner) public onlyExtension  returns (uint    ){
return _createBot(_matronId, _sireId, _generation, _genes, _owner);
}

function extDestroyBot (uint256    _botId) public onlyExtension  {
require(locks[_botId]==0);
_destroyBot(_botId);
}

function extLockBot (uint256    _botId,uint16    _mask) public onlyExtension  {
_lockBot(_botId, _mask);
}

function _lockBot (uint256    _botId,uint16    _mask) internal  {
require(_mask>0);
...
function extUnlockBot (uint256    _botId,uint16    _mask) public onlyExtension  returns (uint16    ){
_unlockBot(_botId, _mask);
}

function _unlockBot (uint256    _botId,uint16    _mask) internal  {
require(_mask>0);
uint16     mask = locks[_botId];
...
function extGetLock (uint256    _botId) public onlyExtension view returns (uint16    ){
return locks[_botId];
}
...
return (_bot.siringWithId==0)&&(_bot.cooldownEndBlock<=uint64 (block.number))&&(locks[_botId]&LOCK_BREEDING==0);
}
...
function isReadyToBreed (uint256    _botId) public view returns (bool    ){
Bot    storage bot = bots[_botId];
return _botId>0&&_isReadyToBreed(_botId, bot);
}

function isPregnant (uint256    _botId) public view returns (bool    ){
return _botId>0&&bots[_botId].siringWithId!=0;
}

function _isValidMatingPair (Bot   storage _matron,uint256    _matronId,Bot   storage _sire,uint256    _sireId) private view returns (bool    ){
...
function breedWithAuto (uint256    _matronId,uint256    _sireId) external whenNotPaused payable {
...
pregnantBots -- ;
bool     __sent_result100 = msg.sender.send(autoBirthFee);
...
event AuctionCreated (address    seller,uint256    tokenId,uint256    startingPrice,uint256    endingPrice,uint256    creationTime,uint256    duration);
...
AuctionCreated(_auction.seller, uint256 (_tokenId), uint256 (_auction.startingPrice), uint256 (_auction.endingPrice), uint256 (_auction.startedAt), uint256 (_auction.duration));
}
...
function bid (uint256    _tokenId) external whenNotPaused payable {
...
contract SaleClockAuction is sGuardPlus,ClockAuction {
...
uint256     sum = 0;
for(uint256     i = 0;i<5; i=add_uint256(i, 1)){
sum=add_uint256(sum, lastGen0SalePrices[i]);
}

return div_uint256(sum, 5);
}
...
function createSaleAuction (uint256    _botId,uint256    _startingPrice,uint256    _endingPrice,uint256    _duration) external whenNotPaused  {
require(_owns(msg.sender, _botId));
require( ! isPregnant(_botId));
_approve(_botId, saleAuction);
saleAuction.createAuction(_botId, _startingPrice, _endingPrice, _duration, msg.sender);
}

function createSiringAuction (uint256    _botId,uint256    _startingPrice,uint256    _endingPrice,uint256    _duration) external whenNotPaused  {
require(_owns(msg.sender, _botId));
require(isReadyToBreed(_botId));
_approve(_botId, siringAuction);
siringAuction.createAuction(_botId, _startingPrice, _endingPrice, _duration, msg.sender);
}

function bidOnSiringAuction (uint256    _sireId,uint256    _matronId) external whenNotPaused payable {
require(_owns(msg.sender, _matronId));
...
contract BotMinting is sGuardPlus,BotAuction {
...
promoCreatedCount=add_uint256(promoCreatedCount, 1);
...
saleAuction.createAuction(botId, _computeNextGen0Price(), 0, GEN0_AUCTION_DURATION, address (this));
gen0CreatedCount ++ ;
}
...
function getBot (uint256    _id) external view returns (bool    isGestating,bool    isReady,uint256    cooldownIndex,uint256    nextActionAt,uint256    siringWithId,uint256    birthTime,uint256    matronId,uint256    sireId,uint256    generation,uint256    genes){
...
{
bool     __sent_result103 = cfoAddress.send(balance-subtractFees);
